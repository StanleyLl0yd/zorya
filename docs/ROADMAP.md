...
- stable-ID multi-tab create/close/reorder product model plus two-phase TabActivationId selection lifecycle, an identity/generation-bound neutral presentation handoff guard and presentation-aware active-tab close coordination; Windows redraws are generation-gated, native tab creation owns one Rarog View per stable TabId, keyboard activation and Ctrl+W close use a confirmed hidden-window neutral fallback, and closed source Views are retired only after fallback presentation; explicit rapid-supersession integration coverage remains tracked in issue #20;
...
- keyboard-first browser commands are modeled and atomically dispatched against committed active-tab identity, including new-tab creation, presentation-aware close, pending-aware tab cycling and reload/stop behavior; Windows maps Ctrl+T, Ctrl+W, Ctrl+Tab and Ctrl+Shift+Tab to browser-owned commands, while Web-content input dispatch remains separate and blocked on issue #16;
