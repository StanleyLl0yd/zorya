...
- stable runtime profile identity and two-phase stale-safe profile selection, with profile preparation kept off the UI thread; typed product settings preserve unknown same-schema entries and storage generations while rejecting malformed known values, and carry a one-in-flight stale-safe persistence lifecycle with monotonic dirty revisions, exact save identity, newer-mutation preservation and failure-safe dirty state; settings save intents now execute only through the bounded `ProfileWorker`, whose nonblocking submission preserves exact one-shot identity for cancellation/retry on queue failure;
