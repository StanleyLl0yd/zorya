...
- stable-ID multi-tab create/close/reorder product model plus two-phase TabActivationId selection lifecycle, an identity/generation-bound neutral presentation handoff guard and presentation-aware active-tab close coordination; Windows redraws are generation-gated and a newer authorized target permit resets worker/compositor retained state before first target presentation, while the observable native neutral-cover phase remains tracked in issue #20 before multi-tab chrome is wired;
