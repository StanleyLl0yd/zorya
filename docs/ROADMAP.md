...
- committed-document authority snapshots are replay-safe before future privileged brokering: `EngineHost::validate_committed_document_authority` resolves the current committed remote authority again through live Host state and returns `true` only for an exact `TabId` + View-generation + navigation-context-token + Site-process-token match; replacement commits, local documents, closed/recreated Views and old snapshots are stale/false, while missing or divergent live Host state remains fail-closed as `InconsistentNavigationState`. Revalidation itself mints no capability, network operation or IPC authority and is not yet capability brokering;
- privileged-request admission is now an explicit deny-by-default chokepoint: `EngineHost::preflight_privileged_request` first revalidates the committed remote-document source, returns `DeniedStaleAuthority` for replayed/replaced/local/closed/recreated authority and only `DeniedUnsupported` for a still-current source requesting product-level `Network` or `Clipboard`; there is intentionally no allow result, no Rarog capability ID/grant, no `rarog-broker` dependency, no Host network operation and no clipboard/IPC action. A later capability-brokering slice must add a separately reviewed allow policy and revalidate immediately before any actual grant;
- crash recovery;
- capability brokering;
