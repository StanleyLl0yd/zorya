...
- stable-ID multi-tab create/close/reorder product model plus two-phase TabActivationId selection lifecycle, an identity/generation-bound neutral presentation handoff guard and presentation-aware active-tab close coordination; Windows redraws are generation-gated, native tab creation owns one Rarog View per stable TabId, and keyboard activation uses a confirmed hidden-window neutral fallback before target presentation; presentation-aware native close and rapid-supersession integration coverage remain tracked in issue #20;
