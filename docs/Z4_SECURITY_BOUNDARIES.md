...
Network requests consume through `preflight_network_once` and then the canonical target-aware policy described below. Clipboard requests consume through `preflight_clipboard_once`; only after one-shot removal and exact envelope equality does a private Clipboard-specific Host helper revalidate the committed remote-document authority. That helper returns only `EngineClipboardRequestDecision::DeniedStaleAuthority` or `DeniedUnsupported`. It is not public admission API and cannot be called with an internal-page authority.

These denial results are Zorya product policy state only. They are not Rarog `CapabilityClass` values, capability IDs, grants or IPC authority. No path here adds `rarog-broker`, grants a capability, starts a Host network operation, calls a clipboard service or makes possession of a current snapshot sufficient authorization. A future brokering/backend slice must define a separately reviewed allow policy and repeat the required live revalidation immediately before any actual grant or operation.
