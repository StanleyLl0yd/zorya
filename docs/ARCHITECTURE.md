...
Windows now also owns the concrete history-save host policy: 5,000 ms debounce, 30,000 ms maximum dirty age and 32 unsaved mutations. A process-local `Instant` origin supplies monotonic scheduler milliseconds. Navigation commit callbacks only mutate in-memory history and non-blockingly submit scheduler intents to `ProfileWorker::save_history`; they never execute profile filesystem work. `ApplicationHandler::about_to_wait` maps the scheduler's next deadline to `ControlFlow::WaitUntil`, otherwise uses `ControlFlow::Wait`; `ControlFlow::Poll` is never used for history persistence. If bounded submission fails, the exact runtime save ownership is cancelled before retry/error handling so unsaved mutations remain dirty.
