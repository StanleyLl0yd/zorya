...
`ProfileCatalog` is likewise a synchronous background-worker primitive. It scans only the product `Profiles` parent, with independent bounds on total directory entries, discovered profile directories and per-profile identity entries. Symlinked catalog entries fail closed, persisted identities must be unique across discovered roots, and a missing identity is accepted only for the legacy directory named `Default` so an existing installation can be upgraded by the normal locked preparation path. Catalog discovery never assigns process-local `ProfileId` values and never treats a path or display label as durable identity.
...
`ProfileWorker` is the dedicated bounded storage-I/O boundary for profile work. Locked profile preparation now also owns legacy identity bootstrap, so the native event loop never creates or rewrites profile identity state. Caller submission uses a one-slot synchronous queue through non-blocking `try_send`; queue-full and disconnected errors return the exact unsubmitted intent to preserve one-shot ownership. The named `zorya-profile` thread consumes selection intents with `PreparedProfile::load`, settings-save intents with `ProfileSettingsSaveIntent::execute`, and history-save intents with `ProfileHistorySaveIntent::execute`, then invokes a caller-provided completion handler with the exact typed selection/save identity. The Windows native shell derives its default profile locator as `%LOCALAPPDATA%\Zorya\Profiles\Default`, begins one exact `ProfileRuntime` selection, and submits that preparation only after winit resumes. The worker completion is forwarded through the existing `EventLoopProxy`; only an exact successful selection commit permits native/Rarog initialization to proceed.
...
User-triggered native profile-selection UX, private-browsing history policy, bookmarks/session stores, broader settings wiring and migrations beyond schema v1 remain separate Z3 slices.
