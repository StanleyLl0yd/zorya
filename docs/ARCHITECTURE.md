...
`PreparedProfile::load` opens both recoverable settings and browsing-history state on the background/profile-worker side. A successful exact profile-selection commit transfers the history snapshot and any recovery report into `ActiveProfile`; runtime history access is gated by the same process-local stable `ProfileId` used for settings so a replaced profile cannot be mutated through a stale identity. This does not make filesystem persistence an event-loop operation: committed-navigation recording, save scheduling/batching and save-completion reconciliation remain separate work.

Native profile-selection UX, profile locking, committed-navigation history recording/persistence scheduling, bookmarks/session stores, broader settings wiring and migrations beyond schema v1 remain separate Z3 slices.
