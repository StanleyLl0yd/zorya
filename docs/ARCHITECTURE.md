...
Native shutdown is two-phase for an active profile. The shell first stops browser/render activity while retaining the profile worker, requests scheduler `Flush` urgency and keeps the event loop waiting for the exact typed history completion. A successful completion is reconciled into `ProfileRuntime`; if mutations newer than the completed snapshot remain dirty, shutdown immediately submits another flush and waits again. Only a clean runtime with no pending history save may drop the profile worker and exit. A storage/reconciliation failure is surfaced as fatal rather than silently reporting a clean shutdown. This keeps all history/profile filesystem I/O off window, input, navigation and shutdown callbacks while still providing durable graceful-exit behavior.

Native profile-selection UX, profile locking, private-browsing history policy, bookmarks/session stores, broader settings wiring and migrations beyond schema v1 remain separate Z3 slices.
