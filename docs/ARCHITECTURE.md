...
Settings persistence has its own one-in-flight runtime lifecycle. `ActiveProfile` tracks a process-local monotonic settings mutation revision and the exact revision known durable; loaded settings start clean, while mutable settings access conservatively advances the revision before exposing the typed object so untracked changes cannot appear clean. `ProfileSettingsSaveId` binds a cloned raw snapshot, active `ProfileId`, profile root, base storage generation and captured mutation revision. The synchronous `ProfileSettingsSaveIntent::execute` call remains an explicit background-worker operation. Exact successful completion advances only the in-memory settings generation and captured durable revision, preserving newer typed mutations for a later save; storage failure leaves the generation unchanged and all mutations dirty. Profile replacement invalidates pending settings-save ownership, stale completion/cancellation cannot target a replacement profile, and exact cancellation clears only in-flight ownership so a failed future worker submission can retry without losing dirty state. Native save timing and worker dispatch remain separate.
