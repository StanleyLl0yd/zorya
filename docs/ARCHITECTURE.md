...
### Profile session startup restoration
...
`ProfileCatalog` is likewise a synchronous background-worker primitive. It scans only the product `Profiles` parent, with independent bounds on total directory entries, discovered profile directories and per-profile identity/metadata entries. Symlinked catalog entries fail closed, persisted identities must be unique across discovered roots, and missing identity or display metadata is accepted only for the legacy directory named `Default` so an existing installation can be upgraded by the normal locked preparation path. Display names live in separate bounded/versioned `.zorya-profile.meta` immutable generation records bound to the exact `ProfileStorageId`; names are validated UTF-8 user text and are never filesystem locators or durable identity. Rename publishes the next metadata generation only under an acquired exact profile lock and checks both storage identity and expected metadata generation, so stale or copied targets fail closed without moving the profile root. Catalog discovery never assigns process-local `ProfileId` values and never treats a path or display label as durable identity.

Profile ownership is explicit and fail-closed. Preparation first creates the profile-root `.zorya-profile.lock` ownership directory and a bounded owner record containing the process id plus a process-local monotonic owner id. Directory creation is the atomic exclusion point: an existing valid owner returns `Held`, while malformed/incomplete ownership state is reported as corruption rather than guessed away. There is no timeout, heartbeat or implicit lease-steal rule. Crash leftovers remain locked until an explicit `recover_abandoned` call names the exact owner that was previously observed; recovery rechecks that owner before removing its unique record, so a stale token cannot delete a replacement owner's lock.

`PreparedProfile` acquires ownership before reading profile identity, display metadata, settings, history, bookmarks or session restore. Missing identity and metadata are bootstrapped only while that exact lock is held: identity uses one immutable versioned entry under `.zorya-profile.id`, while metadata starts at generation 1 with a bounded display name derived from the legacy root label. Empty directories left by interrupted bootstrap are retryable, while malformed, multiple, identity-mismatched or unsupported-version records fail closed. If preparation then fails, it explicitly releases that exact lock on the preparation worker before returning the error. A successful preparation transfers the lock token into `ActiveProfile`. Settings, history, bookmark and session-restore save intents clone that exact token; all four stores verify the token/root before doing write work and again immediately before immutable-generation publication. The token destructor performs no filesystem work. Normal native shutdown first flushes settings/history/bookmarks/session restore, then submits exact lock release through the same bounded `ProfileWorker` and waits for its typed completion before exiting. An abnormal process exit can therefore leave a conservative stale lock, which is preferable to silently enabling a second writer.

`ProfileRuntime` is the portable product-side coordinator for runtime profile selection. Persisted profile storage identity is separate from both process-local `ProfileId` and filesystem paths: each prepared profile carries an immutable `ProfileStorageId` plus the matching current `ProfileMetadata` generation and transfers both unchanged into `ActiveProfile`. Both catalog discovery and `ProfileRuntime::commit_selection` reject duplicate persisted identities, so a copied profile root cannot be accepted as a distinct live profile merely by bypassing catalog discovery. A selection receives a monotonic `ProfileSelectionId`; beginning a newer selection supersedes only the pending intent and leaves the committed profile authoritative. `PreparedProfile::load` opens and decodes the profile synchronously and therefore belongs on a background/profile worker, not the native UI thread. Only a prepared result carrying the exact current selection identity and target may commit. If an active profile still has exact settings/history persistence work, bookmark save ownership/mutations or session-restore save ownership/mutations not yet durable, replacement is rejected with `ActiveProfileNotDurable` and returns the unchanged `PreparedProfile`; pending save ownership is never invalidated merely to switch profiles. A successful replacement therefore occurs only after the old profile is fully durable, allocates a fresh process-local `ProfileId`, and returns the replaced `ActiveProfile` with its exact lock token still owned by the caller. The Windows shell stages such a replacement by retaining the prepared target, requesting `Flush` urgency until all active profile stores are clean, waiting for browser/native navigation and tab-transition work to become quiescent, retrying the exact commit, and then releasing the replaced profile lock only through `ProfileWorker`; the replacement lifecycle is complete only after the exact typed owner completion. The first user-triggered profile UX is `Ctrl+Shift+M`, which submits bounded catalog discovery through `ProfileWorker`, cycles deterministically by persisted catalog identity, and preserves exact discovery/selection intents across one-slot queue-full retries. A successful live switch never inherits the old profile's tabs or per-tab back/forward state: the native window is first hidden as a neutral presentation boundary, `BrowserApp` replaces the window session with one fresh monotonic `TabId`, the render worker replaces its `EngineHost` on the background thread while retaining the existing GPU/surface, and only a fresh `about:blank` commit can re-enable presentation. The active profile display name is applied to the window title only after that reset succeeds, so retained old pixels are never presented under the new profile label. Every dispatched native navigation carries the exact `ProfileId` that owned it at dispatch, and its eventual history commit is reconciled only against that same identity. A later active profile is never inferred as the owner of an older navigation. While selection/replacement is pending, new browser keyboard commands and new-tab starts are suppressed so the quiescent boundary cannot be extended indefinitely by fresh user work. Queue-full release submission retains equivalent exact ownership for a later event-driven retry. Filesystem paths remain profile locators and are never used as browser profile identity. Stale selections and mutations addressed to a replaced `ProfileId` are rejected.

`ProductSettings` is a typed view over the committed raw `SettingsSnapshot`. Typed mutations preserve the snapshot generation so `ProfileStore` stale-writer protection remains authoritative. Defaults do not materialize new raw keys merely by loading a profile, unknown keys within the supported storage schema are preserved across typed edits, and malformed values for known typed keys fail visibly instead of silently resetting unrelated settings. The first typed keys are browser-chrome color-scheme preference and confirmation before closing multiple tabs. Windows consumes the active profile color-scheme preference at native-window creation and again while a profile replacement is still hidden, mapping System/Light/Dark only through winit's supported window-theme boundary; Ctrl+Shift+L cycles the exact active profile value and hands persistence to the existing bounded settings scheduler. Multi-tab close confirmation and broader settings UI remain separate UX work.

Settings persistence has its own one-in-flight runtime lifecycle. `ActiveProfile` tracks a process-local monotonic settings mutation revision and the exact revision known durable; loaded settings start clean, while mutable settings access conservatively advances the revision before exposing the typed object so untracked changes cannot appear clean. `ProfileSettingsSaveId` binds a cloned raw snapshot, active `ProfileId`, profile root, base storage generation and captured mutation revision. The synchronous `ProfileSettingsSaveIntent::execute` call is executed only by the bounded `ProfileWorker`; `save_settings` uses non-blocking submission and returns the exact unsubmitted intent on queue-full or worker-unavailable failure so runtime ownership can be cancelled before retry. Exact successful completion advances only the in-memory settings generation and captured durable revision, preserving newer typed mutations for a later save; storage failure leaves the generation unchanged and all mutations dirty. Profile replacement is blocked while settings-save ownership is pending or any settings mutation remains dirty; stale completion/cancellation therefore cannot be made harmless by discarding old ownership, and exact cancellation clears only in-flight ownership without losing dirty state. `ProfileSettingsSaveScheduler` adds a clock-injected debounce / maximum-dirty-age / mutation-threshold policy with explicit flush urgency and no storage ownership.
...
`ProfileHistorySaveScheduler` adds deterministic coalescing over that dirty state without owning a clock, timer, thread or storage call. Callers provide monotonic milliseconds and a validated policy containing a debounce, maximum dirty age and mutation threshold. After observing dirty state it exposes the next bounded save deadline, allowing native hosts to schedule an event-driven wakeup rather than poll. Normal polling saves after quiescence, the mutation threshold or the maximum dirty age; explicit flush urgency saves dirty history immediately. While a save is in flight newer mutations are observed but no overlapping intent is created. A successful captured save therefore leaves newer mutations eligible for the next scheduling window. Clock regression is treated conservatively as an immediate dirty save, and exact `ProfileId` validation occurs before scheduler ownership changes so stale profile events cannot retarget replacement state. The scheduler only returns `ProfileHistorySaveIntent`.

`ProfileWorker` is the dedicated bounded storage-I/O boundary for profile work. Locked profile preparation owns legacy identity/metadata bootstrap, so the native event loop never creates or rewrites profile catalog state. Catalog discovery, profile creation and inactive-profile rename are also typed worker commands; their completions return the exact original intent, and queue-full/disconnected submission returns that exact unsubmitted intent. Creation allocates a persisted storage identity, creates its final storage-id-derived root in an unpublished staging state, initializes identity plus metadata under an exact profile lock, explicitly releases the lock, and only then atomically publishes an empty `.zorya-profile.ready` directory. Generated roots without that marker are ignored by discovery but still count against the parent directory scan bound, so a crash cannot surface a half-created profile or create unbounded invisible work. Caller submission uses a one-slot synchronous queue through non-blocking `try_send`; queue-full and disconnected errors preserve one-shot ownership. The named `zorya-profile` thread also consumes selection intents with `PreparedProfile::load`, settings-save intents with `ProfileSettingsSaveIntent::execute`, history-save intents with `ProfileHistorySaveIntent::execute`, bookmark-save intents with `ProfileBookmarksSaveIntent::execute`, and session-restore-save intents with `ProfileSessionRestoreSaveIntent::execute`, then invokes a caller-provided completion handler with exact typed work. The Windows native shell derives its default profile locator as `%LOCALAPPDATA%\Zorya\Profiles\Default`, begins one exact `ProfileRuntime` selection, and submits that preparation only after winit resumes. The worker completion is forwarded through the existing `EventLoopProxy`; only an exact successful selection commit permits native/Rarog initialization to proceed.
...
Native shutdown is staged for an active profile. The shell first stops browser/render activity while retaining the profile worker, requests scheduler `Flush` urgency and keeps the event loop waiting for exact typed settings/history/bookmark/session completions. Successful completions are reconciled into `ProfileRuntime`; if newer mutations remain dirty, shutdown immediately submits another flush and waits again. If shutdown intersects a prepared-but-uncommitted replacement, that exact prepared target is cancelled and its lock is drained as rejected ownership. If shutdown intersects a committed replacement whose old lock has not yet completed worker release, that old lock is drained before any rejected target and before the current active-profile lock. Once all active profile stores are clean and all earlier replacement ownership is resolved, the shell submits the current exact profile-lock token to `ProfileWorker::release_lock` and waits for the matching owner completion before dropping the worker and exiting. A storage, reconciliation or lock-release failure is surfaced as fatal rather than silently reporting a clean shutdown. This keeps all profile filesystem I/O off window, input, navigation and shutdown callbacks while still providing durable graceful-exit behavior.
