...
source = r'''use crate::profile_lock::{ProfileLock, ProfileLockError};
...
impl SessionRestoreStore {
    ...
    pub fn load(&self) -> Result<SessionRestoreLoad, SessionRestoreError> {
        ...
        for generation in generations {
            ...
            match decode_session(&bytes, generation) {
                Ok(snapshot) => {
                    let recovery = (!corrupt_generations.is_empty()).then_some(
    ...
    pub fn save(
        ...
        if let Err(error) = file.write_all(&bytes) {
            ...
            return Err(io_error("write pending session generation", &pending_path, error));
        }
        if let Err(error) = file.sync_all() {
            let _ = fs::remove_file(&pending_path);
            return Err(io_error("sync pending session generation", &pending_path, error));
        }
        drop(file);
    ...
    fn discover_generations_with_reserve(
        ...
        for entry in entries {
            ...
            let file_type = entry
                .file_type()
                .map_err(|error| io_error("inspect session directory entry", &entry.path(), error))?;
    ...
    fn create_pending_file(
        &self,
        generation: u64,
    ) -> Result<(PathBuf, File), SessionRestoreError> {
    ...
    fn session_path(&self, generation: u64) -> PathBuf {
        self.session_directory
            .join(session_file_name(generation))
    }
}
...
fn read_bounded(path: &Path, limit: usize) -> Result<Vec<u8>, SessionRestoreError> {
    let file = File::open(path).map_err(|error| io_error("open session generation", path, error))?;
...
mod tests {
    ...
    fn snapshot_preserves_order_active_tab_and_monotonic_identities() {
        ...
        assert_eq!(snapshot.window(first_window).unwrap().active_tab(), Some(first_tab));
        ...
        let third_tab = snapshot.add_tab(second_window, "https://three.test").unwrap();
        assert_eq!(
            snapshot
                .windows()
                .iter()
        ...
        let removed = snapshot.remove_tab(first_window, second_tab).unwrap().unwrap();
    ...
    fn model_bounds_and_foreign_active_tab_are_rejected_before_mutation() {
        ...
        assert_eq!(snapshot.window(first).unwrap().active_tab(), Some(first_tab));
    }
    ...
    fn codec_round_trips_order_active_state_and_next_identities() {
        ...
        assert_eq!(decoded.window(first).unwrap().active_tab(), Some(second_tab));
        assert_eq!(decoded.add_window().unwrap().get(), 3);
        assert_eq!(decoded.add_tab(second, "https://next.test").unwrap().get(), 4);
    ...
    fn pending_files_are_ignored_and_directory_scans_are_bounded() {
        let directory = TestDirectory::new();
        let store = SessionRestoreStore::open(directory.path()).unwrap();
        let pending = store.session_directory.join(
            ".pending-session-00000000000000000001-0000000001-00000000000000000001.tmp",
        );
...
if path.exists():
    raise SystemExit(f"{path}: unexpected pre-existing path")
